              ...
              <form method="post" action="{{ route('roles.update', ['role' => $role->id]) }}">
                ...
                <div class="row mb-3">
                  ...
                  <label for="nameRole" class="col-sm-2 col-form-label fw-bold">Name role</label>
                    <input type="text" value="{{ $role->name_role }}" class="form-control" id="nameRole" name="name_role">
                  </div>
                </div>
                          ...
                          <div class="row">
                            @foreach($value['group'] as $group)
                              @php
                               $checked = ''; 
                              @endphp
                              @foreach ($getPermissionRoles as $role)
                              ...
                              @endforeach
                              <div class="col-md-3">
                                <label><input style="margin-right: 3px;" {{ $checked }} value="{{ $group['id'] }}" type="checkbox" name="permission_id[]">{{ $group['name'] }}</label>
                              </div>
                            @endforeach
                          </div>
                        </div>
